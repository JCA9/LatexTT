...
accesibilidad de la información [A1].
...
y la digitalización lo convierte en algo editable, reconociendo el texto que contiene [A2]. 
...
instantánea de la información deseada posteriormente [A2]. Dicho de otra forma, es el proceso por el cual se convierte una imagen de texto en un formato de texto que pueden 
leer las maquinas . 
...
se devolverían todas las órdenes de pago, las facturas y los formularios que se enviaron originalmente como documentación [A3].
...
procesamiento de lenguaje natural, como la clasificación de documentos, el resumen, el análisis de opiniones, el modelado de temas, el reconocimiento de entidades, etc. [A3].
...
bases de datos sin necesidad de ingresarlos manualmente. [A3].
...
y las áreas oscuras como el texto. [A7]
...
Después del análisis, el sistema convierte los datos de texto extraídos en documentos de texto que pueden leer las máquinas [A3].
\end{itemize}
...
Actualmente hay 4 tipos de programas de OCR que son cada vez más sofisticados y avanzados.[A7]
...
\begin{enumerate}
    ...
    \item El PDF es un conjunto de imágenes o escaneos, aquí el flujo debe ser el aplicar un OCR como lo mencionado anteriormente en la sección 3.1. [A8][A9]
\end{enumerate}
...
páginas relevantes, extracción de tablas y modelos de clasificación para obtener datasets de calidad a gran escala.[A10]
...
obtener la información proveniente de estos documentos es parseando el XML. [A11]
...
a lograr tus metas, el consultor también deberá ayudarte a resolver tus dudas y acompañarte año con año para llegar a tus objetivos.[A12][A13]


    \subsubsection{Metas financieras}
    El banco BBVA recomienda establecer metas financieras siguiendo los siguientes consejos: [A12]
    \begin{itemize}
...
\subsection{Modelos y metodologías de gestión financiera}
    ...
    basándose en su propia experiencia de quiebra en la juventud. [A14]
...
objetivos financieros (ahorro, inversión, reducción de deudas) y corregir desviaciones a tiempo, manteniendo estabilidad y disciplina. [A15]
...
demanda demanda de Inteligencia Artificial, Internet de las Cosas y el blockchain son razones por las que los bancos implementan estrategias para la transformación digital. [A16]
...
La digitalización nos permite ademas de toda la seguridad y automatización, el poder acceder a nuevos clientes constantemente, en cuestión de clic un nuevo usuario puede registrar sus datos y comenzar su experiencia dentro de las aplicaciones bancarias, que ademas de tener un crecimiento diario, puesto que con la digitalización, las empresas bancarias pueden tener una retroalimentación diaria de sus servicios, pudiendo mejorar e innovar según las necesidades detectadas en el usuario, brindando así, una experiencia única y eficiente a cada persona. [A16]


\section{Tecnologías Relacionadas}
Las tecnologías relacionadas al OCR incluyen diversas técnicas y herramientas que nos pueden ayudar con la precisión y la aplicación de OCR en diferentes contextos. Podemos ocupar IA, Machine Learning o NLP para conseguir extraer los datos mas exactos y poder clasificarlos de mejor manera. Pero el reto se encuentra en como podemos aplicar estas tecnologías para poder obtener mejor la información del documento/foto a digitalizar.

    \subsection{Inteligencia Artificial y Machine Learning}
La Inteligencia Artificial (IA) y el Machine Learning (ML) son disciplinas fundamentales en el procesamiento de documentos, particularmente en tareas como el reconocimiento óptico de caracteres (OCR) y la extracción de datos. Estas tecnologías permiten mejorar significativamente la precisión y flexibilidad de los sistemas OCR, especialmente en contextos complejos como la lectura de documentos manuscritos, fuentes no estándar o textos con ruido visual.

1. Aplicación de Machine Learning en OCR

El uso de Machine Learning en OCR ha revolucionado la capacidad de los sistemas para reconocer texto de manera precisa. En lugar de depender exclusivamente de plantillas predefinidas (como en los métodos tradicionales de coincidencia de patrones), los modelos basados en ML aprenden a partir de grandes volúmenes de datos. Este enfoque les permite reconocer caracteres y palabras incluso en condiciones no ideales, como imágenes borrosas o distorsionadas.

Redes Neuronales Convolucionales (CNNs): Las CNNs se han utilizado ampliamente en OCR debido a su capacidad para identificar patrones en imágenes. Estas redes, que emulan el proceso de percepción visual en los humanos, se entrenan con grandes cantidades de datos de texto y pueden reconocer con alta precisión caracteres y estructuras complejas de documentos.


Redes Neuronales Recurrentes (RNNs): En el caso de documentos más largos o escritura manuscrita, las RNNs y sus variantes, como las LSTM (Long Short-Term Memory), son esenciales para reconocer secuencias de texto que dependen de un contexto anterior, como es el caso de los párrafos o las líneas de texto. Esto hace que el modelo sea más adecuado para aplicaciones como la reconstrucción de documentos o el análisis de formularios con múltiples campos.


2. Aprendizaje Automático para la Extracción de Datos

El aprendizaje automático no solo mejora el OCR, sino también el proceso de extracción de datos estructurados. Utilizando algoritmos de clasificación y segmentación, las redes neuronales pueden identificar y extraer información clave de documentos, como fechas, nombres, importes o conceptos. Este tipo de modelos permite que los sistemas interpreten la estructura de un documento y extraigan datos sin necesidad de intervención humana.

Clasificación de Documentos: Los modelos de ML, como las máquinas de soporte vectorial (SVM) o las redes neuronales profundas, son entrenados para clasificar documentos en diferentes categorías (por ejemplo, facturas, contratos, informes), lo que facilita la organización automatizada de los archivos y la posterior búsqueda de información.


Segmentación y Extracción de Datos: Los modelos de segmentación semántica pueden ser utilizados para identificar áreas relevantes dentro de los documentos (como tablas, textos o campos específicos) y extraer datos de forma estructurada. Los algoritmos de Deep Learning permiten incluso extraer información de documentos con variaciones en el diseño, como facturas con diferentes estructuras de formato.

3. Ventajas de la IA y ML en el Procesamiento de Documentos

El uso de IA y ML en el procesamiento de documentos ofrece varias ventajas:

Mayor precisión: Los sistemas basados en ML son menos propensos a errores que los métodos tradicionales, especialmente cuando se entrenan con grandes conjuntos de datos. Esto es esencial en contextos como el procesamiento de facturas o la gestión de documentos fiscales, donde la precisión es crítica.

Automatización: La capacidad de automatizar tareas complejas como la extracción de datos, clasificación de documentos y indexación permite reducir la intervención humana y agilizar procesos que antes requerían mucho tiempo.

Adaptabilidad: Los sistemas basados en IA pueden aprender y mejorar con el tiempo a medida que se procesan más documentos, adaptándose a nuevos tipos de formatos y mejorando su rendimiento.
    
    \subsection{Procesamiento de lenguaje natural (NLP)}
El procesamiento de lenguaje natural o PLN es una rama de la Inteligencia Artificial que permite a las computadoras reconocer, comprender y generar texto y voz mediante la combinación de la lingüística computacional, el machine learning y el aprendizaje profundo [A4]. El procesamiento de lenguaje natural es clave para analizar estos datos y obtener información empresarial práctica. Las organizaciones pueden clasificar, ordenar, filtrar y comprender la intención o el sentimiento oculto en los datos lingüísticos. [A5]
    
    \subsection{Sistemas de reconocimiento en dispositivos móviles}
El cambio de las soluciones basadas en computadoras de escritorio a las tecnologías de OCR basadas en dispositivos móviles para el escaneo de documentos marca una evolución significativa en la satisfacción de las necesidades de escaneo. Inicialmente, el reconocimiento óptico de caracteres (OCR) se limitaba en gran medida al entorno de escritorio, pero se ha ampliado al escaneo de documentos y a los escaneos a través de aplicaciones móviles en un dispositivo móvil [A6].