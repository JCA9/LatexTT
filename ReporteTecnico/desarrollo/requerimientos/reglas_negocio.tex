...
Los usuarios pueden acceder simultáneamente desde la aplicación móvil y la plataforma web con las mismas credenciales. \\
...
RN11 & Corrección y auditoria de datos &
Todos los cambios realizados por el usuario a los datos extraídos por OCR quedan registrados en la bitácora del sistema para auditoria. \\

\bottomrule
\end{tabularx}
\caption{Reglas de Negocio del Sistema (RN01–RN11)}
\end{table}
...
RN12 & Límite de categorías personalizadas &
...
RN18 & Registro sin monto mínimo &
No existe monto mínimo para registrar ingresos o egresos. Se acepta cualquier valor mayor a cero. \\

RN19 & Período de modificación &
Las transacciones (ingresos y egresos) pueden modificarse únicamente durante los primeros 7 días después de su registro. \\

RN20 & Ingresos recurrentes &
El sistema permite configurar ingresos recurrentes mensuales que se registran automáticamente (ej: salario, mesada). \\

\bottomrule
\end{tabularx}
\caption{Reglas de Negocio del Sistema (RN12–RN21)}
\end{table}
...
RN23 & Tasas de interés manuales &
Las tasas de interés de las deudas deben ser ingresadas manualmente por el usuario según la información del acreedor. \\

RN24 & Deudas sin interés &
El sistema permite registrar deudas sin tasa de interés para préstamos familiares o acuerdos sin costo financiero. \\

RN25 & Alertas de vencimiento &
...
RN26 & Límite diario de notificaciones &
El sistema envía máximo 5 notificaciones regulares por día para evitar saturación del usuario. \\

RN27 & Alertas urgentes prioritarias &
Las alertas urgentes (vencimiento mismo día, superar 100\% presupuesto) pueden superar el límite diario de notificaciones. \\

RN28 & Configuración personalizable &
Los usuarios pueden habilitar o deshabilitar tipos específicos de alertas por módulo (presupuestos, deudas, reportes). \\

RN29 & Alertas de presupuesto &
Se envían alertas cuando el gasto acumulado supera el 80\% del presupuesto configurado por categoría. \\

RN30 & Recordatorios de deuda &
Las alertas de deuda se configuran con anticipación personalizable (3, 7, 15 días antes del vencimiento). \\

\bottomrule
\end{tabularx}
\caption{Reglas de Negocio del Sistema (RN22–RN32)}
\end{table}
...
RN31 & Períodos de reporte &
Los reportes se generan por día, semana, mes o año según la disponibilidad de la plataforma (móvil para reportes simples, web para reportes complejos). \\

RN32 & Rango de fechas limitado &
Los reportes se generan únicamente para fechas donde existan transacciones registradas en el sistema. \\

RN33 & Formatos de exportación &
Los reportes pueden exportarse en formato PDF para visualización o Excel para análisis adicional. \\

RN34 & Reportes por plataforma &
Los reportes complejos y anuales se generan únicamente en la plataforma web. La aplicación móvil maneja reportes básicos y mensuales. \\

\midrule
\multicolumn{3}{c}{\textbf{INTEGRIDAD Y SEGURIDAD DE DATOS}} \\
\midrule

RN35 & Eliminación irreversible &
